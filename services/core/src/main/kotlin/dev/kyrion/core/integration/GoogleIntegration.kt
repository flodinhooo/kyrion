...
class GoogleIntegrationService(
    ...
    fun complete(ownerId: UUID, code: String, state: String): GoogleStatus { val request = pending.remove(state) ?: throw GoogleInvalidStateException(); if (request.first != ownerId || request.second.isBefore(clock.instant())) throw GoogleInvalidStateException(); val token = client.exchange(code, redirectUri, clientId, clientSecret); val now = clock.instant(); val existing = repository.findAll(ownerId).firstOrNull { it.provider == GOOGLE_PROVIDER && it.displayName == token.accountName }; val id = existing?.id ?: UUID.randomUUID(); val payload = mapper.writeValueAsString(token); val protected = cipher.protect(payload, "$ownerId:$id:$GOOGLE_PROVIDER"); if (existing == null) repository.save(IntegrationConnection(id, ownerId, GOOGLE_PROVIDER, token.accountName, token.accountName, protected.ciphertext, protected.nonce, protected.version, now, now)) else repository.updateCredential(ownerId, id, protected, now); grants.replace(ownerId, id, setOf(CALENDAR_READ), now); return GoogleStatus(true, true, token.accountName) }
