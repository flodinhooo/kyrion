...
private data class StoredGoogleToken(val accessToken: String, val refreshToken: String?, val expiresAt: Instant, val accountName: String)
...
class GoogleIntegrationService(
    ...
    private fun token(connection: IntegrationConnection): GoogleToken { val value = mapper.readValue(cipher.reveal(connection), StoredGoogleToken::class.java); return GoogleToken(value.accessToken, value.refreshToken, value.expiresAt, value.accountName) }
    private fun saveToken(connection: IntegrationConnection, token: GoogleToken) { val value = StoredGoogleToken(token.accessToken, token.refreshToken, token.expiresAt, token.accountName); val protected = cipher.protect(mapper.writeValueAsString(value), "${connection.ownerId}:${connection.id}:$GOOGLE_PROVIDER"); repository.updateCredential(connection.ownerId, connection.id, protected, clock.instant()) }
    private fun enc(value: String) = URLEncoder.encode(value, StandardCharsets.UTF_8)
}
