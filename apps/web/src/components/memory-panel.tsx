...
export function MemoryPanel() {
  ...
  async function confirm(id: string) { await mutate(`/api/memory/${id}/confirm`, "POST"); }
  ...
  return (
    <div className="settings-link-card memory-panel">
      ...
      <div className="memory-list">
        ...
        {profile?.items.map((item) => <MemoryItem key={item.id} item={item} pending={pending} onConfirm={confirm} onForget={forget} onUpdate={update} />)}
      </div>
    </div>
  );
}
...
function MemoryItem({ item, pending, onConfirm, onForget, onUpdate }: {
  item: PersonalMemory; pending: boolean;
  onConfirm: (id: string) => Promise<void>; onForget: (id: string) => Promise<void>;
  ...
  return <article className="memory-item">
    <div className="memory-badges"><span>{item.status === "proposed" ? t.memoryProposed : t.memoryConfirmed}</span><span className={sensitivity === "sensitive" ? "sensitive" : ""}>{sensitivity === "sensitive" ? t.memorySensitive : t.memoryStandard}</span></div>
    ...
    <div className="memory-item-controls">
      ...
      {item.status === "proposed" && <button type="button" disabled={pending} onClick={() => void onConfirm(item.id)}>{t.memoryConfirm}</button>}
