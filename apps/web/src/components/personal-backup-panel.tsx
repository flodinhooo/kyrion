...
export function PersonalBackupPanel() {
  ...
  const [state,setState]=useState<"idle"|"working"|"error"|"done">("idle");
  async function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); const form=event.currentTarget; const data=new FormData(form);
    const passphrase=String(data.get("passphrase")??"");
    if(passphrase!==data.get("confirmation")){setState("error");return;}
    setState("working");
    try {
  ...
  return <div className="settings-link-card backup-panel"><strong>{t.backupTitle}</strong><small>{t.backupDescription}</small>
    <form onSubmit={submit}><label>{t.backupPassphrase}<input name="passphrase" type="password" minLength={12} maxLength={200} autoComplete="new-password" required /></label>
    ...
    {state==="done"&&<p className="form-success" role="status">{t.backupCreated}</p>}{state==="error"&&<p className="auth-error" role="alert">{t.backupError}</p>}
  </div>;
}
