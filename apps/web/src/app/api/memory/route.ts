import { isMemoryProfile, isPersonalMemory } from "@/features/memory/contracts";
...
export async function GET() {
  ...
  try {
    const response = await fetch(`${CORE_SERVICE_URL}/v1/memory`, {
      headers: { Authorization: `Bearer ${auth.token}` }, cache: "no-store",
    });
    const value: unknown = await response.json();
    return response.ok && isMemoryProfile(value)
...
export async function POST(request: Request) {
  ...
  try {
    const response = await fetch(`${CORE_SERVICE_URL}/v1/memory/proposals`, {
      method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${auth.token}` },
      body: JSON.stringify(body), cache: "no-store",
    });
    const value: unknown = await response.json().catch(() => ({}));
    return response.ok && isPersonalMemory(value)
