...
export async function PATCH(request: Request, context: RouteContext<"/api/integrations/nanoleaf/connections/[id]">) {
  ...
  try {
    const response = await fetch(`${CORE_SERVICE_URL}/v1/integrations/nanoleaf/connections/${id}`, { method: "PATCH", headers: { "Content-Type": "application/json", Authorization: `Bearer ${auth.token}` }, body: JSON.stringify(body), cache: "no-store" });
