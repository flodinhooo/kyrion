...
export async function PUT(request: Request, context: RouteContext<"/api/integrations/nanoleaf/connections/[id]/scenes/select">) {
  ...
  try {
    const response = await fetch(`${CORE_SERVICE_URL}/v1/integrations/nanoleaf/connections/${id}/scenes/select`, { method: "PUT", headers: { "Content-Type": "application/json", Authorization: `Bearer ${auth.token}` }, body: JSON.stringify(body), cache: "no-store" });
    const value: unknown = await response.json().catch(() => ({}));
    return response.ok && isNanoleafScenes(value) ? Response.json(value) : Response.json(value, { status: response.status });
