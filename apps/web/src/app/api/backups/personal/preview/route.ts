...
export async function POST(request: Request) {
  ...
  try{
    const response=await fetch(`${CORE_SERVICE_URL}/v1/backups/personal/preview`,{method:"POST",headers:{Authorization:`Bearer ${auth.token}`,"Content-Type":"application/json"},body:JSON.stringify(body),cache:"no-store"});
