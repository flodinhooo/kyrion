...
export async function GET(_: Request, context: RouteContext<"/api/gateways/[id]/zigbee/devices">) {
  const auth = await requireApiSession();
  if (auth instanceof Response) return auth;
  const { id } = await context.params;
  try {
    const response = await fetch(`${CORE_SERVICE_URL}/v1/gateways/${id}/zigbee/devices`, {
...
export async function POST(request: Request, context: RouteContext<"/api/gateways/[id]/zigbee/devices">) {
  const auth = await requireApiSession();
  if (auth instanceof Response) return auth;
