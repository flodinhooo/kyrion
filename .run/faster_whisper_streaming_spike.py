...
def transcribe(model: WhisperModel, pcm: np.ndarray, language: str) -> str:
...
def main() -> None:
    ...
    transcribe(model, inputs[0][1], args.language)  # Lazy runtime warm-up outside measurements.
    ...
    for path, full_pcm in inputs:
        ...
        for available in range(step_samples, len(full_pcm) + step_samples, step_samples):
            ...
            text = transcribe(model, full_pcm[window_start:available], args.language)
            decode_seconds = time.perf_counter() - decode_started
            updates.append(
                {
                    "audioAvailableSeconds": round(available / TARGET_RATE, 3),
                    "decodeSeconds": round(decode_seconds, 3),
                    "emittedAtSeconds": round(time.perf_counter() - started_at, 3),
